            ...
            <div class="card">
                ...
                >
                    ...
                    <Column field="inventoryStatus" header="Status" :sortable="true">
                        <template #body="slotProps">
                            ...
                            <span :class="'condpgto-badge status-' + (slotProps.data.status ? slotProps.data.status.toLowerCase() : '')">{{ slotProps.data.status }}</span>
                        </template>
                    </Column>
                ...
                <Dialog v-model:visible="editDialog" :style="{ width: '450px' }" header="Editar" :modal="true" class="p-fluid">
                    <div class="p-field">
                        ...
                        <InputText id="descricao" v-model.trim="condpgto.descricao" required="true" autofocus :class="{ 'p-invalid': submitted && !condpgto.nome }" />
                        ...
                        <Dropdown
                            ...
                            <template #value="slotProps">
                                <div v-if="slotProps.value && slotProps.value.value">
                                    <span :class="'condpgto-badge status-' + slotProps.value.value">{{ slotProps.value.label }}</span>
                                </div>
                                <div v-else-if="slotProps.value && !slotProps.value.value">
                                    <span :class="'condpgto-badge status-' + slotProps.value.toLowerCase()">{{ slotProps.value }}</span>
                                </div>
                                <span v-else>
                    ...
                    <div class="p-formgrid p-grid">
                        ...
                        <div class="p-field p-col">
                            ...
                            <InputNumber id="valormin" v-model="condpgto.valormin" mode="currency" currency="BRL" locale="pt-BR" />
                        </div>
                ...
                <Dialog v-model:visible="deleteDialog" :style="{ width: '450px' }" header="Confirmar" :modal="true">
                    <div class="confirmation-content">
                        <i class="pi pi-exclamation-triangle p-mr-3" style="font-size: 2rem" />
                        <span v-if="condpgto">Confirma deletar todos os registros selecionados?</span>
                    </div>
                    <template #footer>
...
export default {
    ...
    methods: {
        ...
        save() {
            this.submitted = true;
            if (
                this.condpgto.descricao
                && this.condpgto.status
            ) {
                this.loadingTable = true;
                if (this.condpgto.id) { // Save editted condpgto
                    this.condpgto.status = this.condpgto.status.value ? this.condpgto.status.value : this.condpgto.status;

                    this.service.update(this.condpgto).then((data) => {
                        this.condpgtos[this.findIndexById(data.id)] = data;
                        this.$toast.add({ severity: 'success', summary: 'Successo', detail: 'Condição Atualizada', life: 3000 });
                        this.editDialog = false;
                        this.condpgto = {};
                        this.loadingTable = false;
                    });
                    ...
                    this.condpgto.status = this.condpgto.status ? this.condpgto.status.value : 'A';
                    this.service.store(this.condpgto).then((data) => {
                        this.condpgtos.unshift(data);
                        this.$toast.add({ severity: 'success', summary: 'Successo', detail: 'Condição Criada', life: 3000 });
                        this.editDialog = false;
                        this.condpgto = {};
                        this.loadingTable = false;
                    });
        ...
        delete() {
            this.service.delete([this.condpgto.id]).then((id) => {
                this.condpgtos = this.condpgtos.filter((val) => val.id !== Number(id[0]));
                this.showDeleteDialog = false;
                this.condpgto = {};
                this.$toast.add({ severity: 'success', summary: 'Successo', detail: 'Condição Deletado', life: 3000 });
            });
        },
        // Find condpgto by id
        findIndexById(id) {
        ...
        deleteSelectedRegs() {
            ...
            this.service.delete(ids).then((id) => {
                this.condpgtos = this.condpgtos.filter((val) => !id.includes(val.id.toString()));
