            ...
            <div class="card">
                ...
                <Toolbar class="p-mb-4">
                    <template v-slot:left>
                            ...
                            <Button label="Novo" icon="pi pi-plus" class="p-button-success p-mr-2" @click="novoPedido"/>
                ...
                >
                    ...
                    <Column field="valor" header="Valor" :sortable="true" class="p-text-right">
                        <template #body="slotProps">
                    ...
                    <Column>
                        <template #body="slotProps">
                            <router-link :to="{ name: 'pedidoeditar', params: { id: slotProps.data.id }}"  class="logo">
                            <Button icon="pi pi-eye" class="p-button-rounded p-button-success p-mr-2"  />
                            </router-link>
                            <Button icon="pi pi-ban" v-if="slotProps.data.status == 'A'" class="p-button-rounded p-button-danger" @click="confirmarDelete(slotProps.data)" />
                        </template>
                    </Column>
                    ...
                    <template #expansion="slotProps">
                        <div class="orders-subtable">
                            ...
                            <DataTable
                                ...
                                :loading="slotProps.data.itens != null && slotProps.data.itens.length > 0 ? false : loadingTableItens"
                                responsiveLayout="scroll"
                            >
                                ...
                                <Column headerStyle="width:4rem">
                                    <template #body>
                                        <Button icon="pi pi-eye" @click="visualizarItem(slotProps.data)"/>
                                    </template>
                                </Column>
                            </DataTable>
                        </div>
                ...
                <Dialog v-model:visible="pedidoItemDialog" :style="{ width: '450px' }" header="Visualizar Item Pedido" :modal="true" class="p-fluid">

                    <div class="p-field">
                        <label for="descricao">Produto</label>
                        <InputText id="descricao" v-model.trim="pedidoItem.descprod" required="true" autofocus :class="{ 'p-invalid': submitted && !pedidoItem.descprod }" :disabled="true" />
                        <small class="p-invalid" v-if="submitted && !pedidoItem.descprod">Produto é obrigatório.</small>
                    </div>
                    ...
                    <div class="p-formgrid p-grid">
                        <div class="p-field p-col">
                            ...
                            <InputText id="pedidoitemobs" v-model="pedidoItem.obs" :disabled="true"/>
                        </div>
                    ...
                    <div class="p-formgrid p-grid">
                        <div class="p-field p-col">
                            ...
                            <InputNumber id="pedidoitemquant" v-model="pedidoItem.quant" :disabled="true"/>
                        </div>

                        <div class="p-field p-col">
                            <label for="valormin">Valor</label>
                            <InputNumber id="pedidoitemvalor" v-model="pedidoItem.vlrtotal" mode="currency" currency="BRL" locale="pt-BR" :disabled="true"/>
                        </div>
                    </div>
                ...
                <Dialog v-model:visible="showCancelaDialog" :style="{ width: '450px' }" header="Confirmar" :modal="true">
                    <div class="confirmation-content">
                        <i class="pi pi-exclamation-triangle p-mr-3" style="font-size: 2rem" />
                        <span v-if="pedido"
                        >Tem certeza de que deseja cancelar o pedido <b>{{ pedido.id }}</b
                        > - <i>{{ pedido.nomecli }}</i>?</span
                        >
                    </div>
                    <template #footer>
                        <Button label="Não" icon="pi pi-times" class="p-button-text" @click="showCancelaDialog = false" />
                        <Button label="Sim" icon="pi pi-check" class="p-button-text" @click="cancelaPedido" />
                    </template>
                </Dialog>
    ...
    methods: {
        ...
        save() {
            ...
            if (this.pedido.descricao.trim()) {
                if (this.pedido.id) { // Save editted pedido
                    this.pedido.status = this.pedido.status.value ? this.pedido.status.value : this.pedido.status;
                    ...
                    this.pedido.status = this.pedido.status ? this.pedido.status.value : 'A';
